...
def extract_nodes_dependent_on_predicate(tree, predicate):
  """Returns the set of AST nodes dependent on nodes matching `predicate`.

  Notice we adopt the convention that a node that satisfies the predicate
  is dependent on the predicate.
  ...
  Args:
    ...
    predicate: One-arg callable, accepting arguments of type
      ...
      Boolean indicating match or mismatch with the desired pattern.
  ...
  Returns:
    ...
    Notice that since TFF is functional, this set will not be robust to
    transformations performed on `tree`. This module is designed to contain
    static analysis, performed independently of any transformations.
  """
...
def is_broadcast_dependent_on_aggregate(tree):
  """Looks for a broadcast in `tree` which ingests the result of an aggregate.

  Returns in addition a list of examples of nodes where the result of an
  aggregate is being wired into a broadcast.
  ...
  Returns:
    A two-tuple, whose first element is a Boolean indicating whether `tree`
    contains any broadcasts which ingest the results of aggregations, and whose
    second element is a list of examples of this pattern.
  """
  ...
  nodes_dependent_on_aggregate = extract_nodes_dependent_on_predicate(
      tree, aggregate_predicate)

  nodes_dependent_on_broadcast = extract_nodes_dependent_on_predicate(
  ...
  return broadcast_dependent, examples
